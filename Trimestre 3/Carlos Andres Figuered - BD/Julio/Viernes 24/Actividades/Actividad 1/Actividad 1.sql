...
SELECT * FROM Pacientes 
WHERE fecha_nacimiento >= '2000-01-01' AND NOT (nombre = 'María');

-- 4. Obtener los pacientes cuyo teléfono sea '555-0101' O '555-0105'.
SELECT * FROM Pacientes 
WHERE telefono = '555-0101' OR telefono = '555-0105';
...
SELECT * FROM Pacientes 
WHERE NOT (documento_identidad = '101010101' OR documento_identidad = '101010102');
...
SELECT * FROM Citas 
WHERE (id_paciente = 1 OR id_paciente = 2) AND estado = 'Programada';
...
SELECT * FROM Citas 
WHERE (estado = 'Cancelada' OR estado = 'No asistió') AND id_medico = 3;
...
SELECT * FROM Citas 
WHERE fecha_hora >= '2026-05-02 00:00:00' AND fecha_hora <= '2026-05-02 23:59:59';

-- 16. Buscar citas del paciente ID 5 donde el estado NO sea 'Cancelada'.
SELECT * FROM Citas 
WHERE id_paciente = 5 AND NOT (estado = 'Cancelada');

-- 17. Obtener las citas asignadas al médico ID 4 o ID 9, SIEMPRE QUE su estado sea 'Completada'.
SELECT * FROM Citas 
WHERE (id_medico = 4 OR id_medico = 9) AND estado = 'Completada';
...
SELECT * FROM Citas 
WHERE fecha_hora >= '2026-06-01 00:00:00' AND id_paciente = 1 AND estado = 'Programada';
...
SELECT * FROM Historiales_Medicos 
WHERE fecha_registro > '2026-05-05 00:00:00' AND id_cita < 10;
